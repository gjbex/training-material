...
! get chunk size and check it
call get_chunk_size(nr_chunk_rows)
if (mod(nr_rows, nr_chunk_rows) /= 0) then
    write (unit=error_unit, fmt="(A)") &
        "chunk size should divide matrix rows"
    stop
end if
